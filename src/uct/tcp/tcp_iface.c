                              ...
                              void *arg)
{
    ...
    for (;;) {
        ...
        status = uct_tcp_cm_handle_incoming_conn(iface,
                                                 ...
                                                 fd);
        if (status != UCS_OK) {
          ucs_warn("closing fd after accept");
            ucs_close_fd(&fd);
            return;
...
static ucs_status_t uct_tcp_iface_server_init(uct_tcp_iface_t *iface)
{
    ...
    /*if (iface->config.enable_nat_traversal) {
      ...
      if (getsockname(fd, (struct sockaddr *)&rend_local_addr, &rend_local_addr_len) < 0) {
                                   ...
                                   unsigned *num_devices_p)
{
    ...
    if (status == UCS_ERR_IO_ERROR) {
      ...
      ++num_devices;
      ucs_warn("processed fake interface for aws lambda support");
